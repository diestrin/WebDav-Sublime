...
class Upload(object):
	def __init__(self, paths=None, remote_path=None, config_path=None, pathToDownload=None, resource=None, console=None, callback=None):
		if paths:
			...
			self.console.log(self.project_root + self.local_path)
			if not os.path.exists(self.project_root + self.local_path):
				self.console.end_loading("Error: no existe la ruta")
				return

			self.thread = Threads.WebdavUploadFile(resource[0], self.project_root + self.local_path)
			self.thread.start()
			self.handle_thread()
